...
export const AddContractor: React.FC = () => {
  ...
  const onFinishFailed: FormProps<ContractorContextType>["onFinishFailed"] = (
    errorInfo
  ) => {
  ...
  const handleChecked = (e: CheckboxChangeEvent) => {
    ...
    setFormData((prevState) => ({
      ...
      fields: checked
        ...
        : prevState.fields.filter((f) => f !== value),
    }));
  ...
  return (
    <>
      <Form
        ...
        autoComplete="off"
      >
        <h2>Registrace nového řemeslníka</h2>
              ...
              rules={[
                {
                  validator: (_, value) =>
                    formData.fields.length > 0
                      ? Promise.resolve()
                      : Promise.reject(
                          new Error("Vyberte alespoň jednu oblast")
                        ),
                },
              ]}
            >
        ...
        <Form.Item label="Pracuji o víkendech" valuePropName="checked">
          <Switch
            onChange={(checked) =>
              setFormData({ ...formData, weekends: checked })
            }
          />
        </Form.Item>
        ...
        <Form.Item
          label="Pracuji v okolí (km)"
          name="range"
          rules={[{ required: true, message: "Posuvníkem vyberte hodnotu" }]}
        >
